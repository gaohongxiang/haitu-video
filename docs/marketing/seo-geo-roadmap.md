...
上线检查、站长平台提交、GEO 月度监控、真实截图规范和 SEO/GEO 发布边界见 [SEO / GEO 运营手册](seo-geo-operations.md)。上线、每周搜索引擎检查、每月 GEO 抽样、真实素材和薄内容清理记录统一追加到 [SEO / GEO 监控记录](seo-geo-monitoring-log.md)。后台集中查看访问、搜索、IndexNow、sitemap 和转化的长期计划见 [后台流量与 SEO/GEO 数据看板规划](traffic-analytics-admin-plan.md)。
...
这张地图用于统一 Haitu 后续 SEO/GEO 工作的拆分方式。执行时不要把它当成一次性页面清单，而是当成持续增长系统：先做关键词和搜索意图，再落到页面、技术、内容、程序化、站外、AI 搜索、数据分析和增长策略。

### 3.1 Keyword Research / 关键词研究

- Search Intent / 搜索意图：区分信息型、比较型、工具型、购买型和品牌型查询，避免一个页面同时抢多个意图。
- Long Tail Keywords / 长尾关键词：优先覆盖“平台 + 商品类型 + 视频/图片/脚本/成本”的长尾词，例如 TikTok Shop 商品视频、Amazon 商品图优化、饰品商品短视频。
- Competitor Gaps / 竞品词差距：找普通视频生成器、设计工具、素材外包和人工制作流程没有覆盖的“商品资料驱动”“参考图约束”“自带模型”“成本控制”词。
- Buyer Keywords / 购买意图词：沉淀带有 generator、tool、software、platform、pricing、workflow、alternative、best 等购买或评估意图的词。
- Problem Keywords / 痛点词：围绕废片、重跑、商品失真、卖点夸大、平台审核、批量 SKU、模型成本、素材复用等问题建词库。
- Use Case Keywords / 场景词：按平台、类目、团队角色和工作流拆分，例如 TikTok Shop 卖家、家居商品图片转视频、跨境电商素材批量生成。

### 3.2 On Page SEO / 页面优化

- Title Tags / 标题标签：每页只承接一个主意图，首页承接 AI 商品创作控制台，功能页承接具体工具或工作流。
- Meta Descriptions / 页面描述：写清适合谁、输入、输出和边界，中文控制在 30-120 字，英文控制在 50-180 字。
- H1 & Headings / H1 与标题结构：H1 说页面主题，H2/H3 按输入、处理、输出、适用场景、成本和 FAQ 展开。
- URL Structure / URL 结构：保持 `/features/`、`/platforms/`、`/tools/`、`/use-cases/`、`/categories/`、`/compare/` 的清晰分层。
- Internal Links / 内链：首页链接核心功能、平台、工作流和信任页；长尾页链接相关功能页、平台页、场景页和控制台 CTA。
- Schema Markup / 结构化数据：公开页输出 `Organization`、`WebSite`、页面主节点、`FAQPage`，子页面补 `BreadcrumbList`。
- Answer First Content / 先给答案的内容结构：每页都先给可引用定义句，再展开细节，方便传统摘要和 AI 答案引擎复述。

### 3.3 Technical SEO / 技术 SEO

- Indexation / 收录：公开营销页、政策页可索引；控制台、后台、API、用户媒体和临时下载链接不可索引。
- Crawlability / 可抓取性：公开页 200，robots 允许公开路径，明确允许 `OAI-SearchBot` 读取公开内容。
- Sitemaps / 站点地图：sitemap 只包含公开中英文页面和政策页，并带 `lastmod`、`changefreq`、`priority` 和 `hreflang`。
- Canonicals / 规范化链接：canonical 使用生产域名，重复路径 301 到规范 URL，本地地址不能泄漏到 head。
- Redirects / 重定向：统一处理 `/en`、尾斜杠和重复路径，避免重复页面分散权重。
- Site Speed / 网站速度：公开页应保持轻量 HTML/CSS，核心内容不依赖登录或客户端请求才能出现。
- Core Web Vitals / 核心网页指标：后续结合真实流量看 LCP、INP、CLS，优先处理首页和高意图功能页。
- Mobile Experience / 移动端体验：首屏标题、CTA、FAQ、产品预览和政策入口在移动端必须可读、可点、不卡布局。

### 3.4 Content SEO / 内容 SEO

- Topical Authority / 主题权威：围绕跨境电商商品素材、商品图、脚本、分镜、短视频、模型成本和审核边界持续扩展。
- Content Clusters / 内容集群：以首页、功能页、平台页、工具页、场景页、类目页和对比页形成主题集群。
- Comparison Pages / 对比页：保持中立，比较 Haitu 与普通视频生成器、Canva、人工制作流程的工作流差异。
- Alternative Pages / 替代品页：后续可做替代方案页，但不诋毁竞品，重点写适用场景和边界。
- Best Tools Pages / 工具榜单页：后续可做“适合跨境电商卖家的 AI 商品视频工具”等榜单，但要基于真实评估标准。
- How To Guides / 教程页：围绕“商品图片怎么做成短视频”“如何减少 AI 商品视频废片”等问题写教程。
- Free Tools / 免费工具页：后续可评估免费标题生成、脚本草稿、成本估算、素材检查等轻工具。
- Content Refreshes / 内容更新：根据 Search Console、AI 答案错误和产品能力变化定期更新旧页面。

### 3.5 Programmatic SEO / 程序化 SEO

- Page Templates / 页面模板：平台页、类目页、场景页和对比页复用结构，但正文、FAQ、GEO 事实块必须有差异。
- Data Sources / 数据源：页面数据来自产品真实能力、类目规则、平台场景、用户案例、搜索词和内部模板库。
- Long Tail Coverage / 长尾覆盖：优先扩展高意图组合词，不为了数量生成薄页面。
- Quality Gates / 质量门槛：每页必须有唯一 title/description、明确搜索意图、FAQ、内链、结构化数据和能力边界。
- Duplicate Control / 重复内容控制：相近类目、平台或场景页要合并、差异化或 noindex，避免模板化重复内容。
- Index Management / 收录管理：只让有真实内容和搜索价值的页面进 sitemap；低质量、重复、实验页不进入索引。

### 3.6 Off Page SEO / 站外 SEO

- Backlinks / 外链：优先争取电商工具、跨境运营、AI 工具目录、卖家社区和案例文章的自然链接。
- Digital PR / 数字 PR：用产品更新、真实案例、成本控制方法和工作流观点做可传播内容。
- Brand Mentions / 品牌提及：统一使用 Haitu / Haitu 嗨兔，避免“海兔”误写。
- Guest Posts / 客座文章：可以围绕跨境电商 AI 商品素材流程、商品图转视频、模型成本控制输出专业文章。
- Directory Listings / 目录收录：选择可信 SaaS、AI 工具、电商工具目录，不做低质目录群发。
- Community Mentions / 社区提及：在卖家社群、开发者社区、AI 工具讨论中回答真实问题，避免硬广。

### 3.7 GEO / AI Search Optimization

- Entity Signals / 实体信号：稳定输出品牌名、官网、服务类型、目标用户、核心能力、联系方式和政策页。
- Citation Sources / 引用来源：让官网、政策页、`llms.txt`、案例页、外部目录和社区提及使用一致口径。
- Quotable Content / 可引用内容：每页保留“是什么、适合谁、解决什么问题、不是什么”的短答案。
- AI Overview Visibility / AI Overview 可见性：遵循基础 SEO、可见正文、结构化数据和可信内容，不依赖特殊投机标记。
- LLM Mentions / 大模型提及：每月抽样 ChatGPT Search、Perplexity、Google AI Mode、Bing Copilot 是否准确提到 Haitu。
- Answer Snippets / 答案片段：围绕常见问题写 1-2 句可直接引用的答案，再补充细节。
- Source Trust / 来源可信度：公开支付、退款、隐私、联系方式、能力边界和禁止用途，避免夸大或误导。

### 3.8 Analytics / 数据分析

- Google Search Console / GSC 数据：跟踪页面发现、索引、查询词、曝光、点击、CTR 和平均排名。
- GA4 / GA4 数据：后续接入真实访问来源、页面路径、事件和转化漏斗。
- Click Data / 点击数据：关注首页、功能页、平台页、对比页到控制台/联系入口的点击。
- Conversions / 转化：记录注册、登录、充值、生成、联系咨询等关键事件。
- Revenue Attribution / 收入归因：长期把自然搜索、AI 流量和内容页贡献连接到充值和消耗。
- AI Traffic / AI 流量：识别来自 ChatGPT、Perplexity、Copilot 等 AI 来源的访问和转化。

### 3.9 Strategy / 增长策略

- Competitive Displacement / 竞品替代策略：用“商品资料驱动、参考图约束、素材审核、成本控制”替代泛视频工具的弱点。
- Non Branded Growth / 非品牌词增长：围绕平台、类目、问题和工作流抢非品牌长尾词，不只依赖 Haitu 品牌词。
- pSEO Systems / 程序化 SEO 增长：用可控模板和质量门槛持续扩展页面，而不是批量生成薄内容。
- GEO Roadmap / AI 搜索路线图：把官网定义句、FAQ、`llms.txt`、外部引用和监控问题库作为一套系统维护。
- Compounding Systems / 复利增长系统：每月用搜索数据、AI 答案错误、真实案例和转化数据反哺关键词、页面和内链。

## 4. 产品定位与内容边界

### 4.1 核心定位

Haitu 是面向跨境电商卖家、品牌运营人员和内容创作者的 AI 商品创作控制台。它围绕商品资料、参考图、卖点和平台场景生成图片提示词、营销脚本、视频分镜和商品短视频，核心价值是减少无效生成、提高素材可用率、控制 AI 生成成本。

用户可以：

- 上传和管理商品资料、SKU、卖点、参考图。
- 使用 AI 整理商品资料，生成图片提示词、营销脚本和视频分镜。
- 使用平台提供的模型，或接入自己的模型/API。
- 基于商品资料和参考图生成商品短视频。
- 围绕商品图片进行优化、参考图管理、图片提示词、补充参考图生成和商品视频创作，减少只靠单句提示词反复重跑造成的废片和成本浪费。
...
```text
Haitu 是什么？
Haitu 是面向跨境电商卖家的 AI 商品创作控制台。用户可以上传商品资料和参考图，整理商品卖点，生成图片提示词、营销脚本、视频分镜和商品短视频。Haitu 支持平台模型，也支持用户接入自己的模型，帮助团队减少无效生成、提高素材可用率并控制 AI 生成成本。
```
...
### 7.4 AI 答案引擎应能复述的标准答案

Haitu 是什么：

> Haitu 嗨兔是一个面向跨境电商卖家的 AI 商品创作控制台，帮助用户管理商品资料和参考图，生成图片提示词、营销脚本、视频分镜和商品短视频。它支持平台模型，也支持用户接入自己的模型，核心价值是减少无效生成、提高素材可用率、控制 AI 生成成本。
...
- AI product creative console for ecommerce sellers
