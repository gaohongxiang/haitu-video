...
describe("api service mode source", () => {
  ...
  it("lets platform users manage numbered custom bundles after built-in presets", async () => {
    ...
    expect(editableBundleCardSource).toContain('aria-label="保存"');
    expect(editableBundleCardSource).toContain('title="保存"');
    expect(editableBundleCardSource).toContain('title="删除"');
    ...
    expect(bundleStatusToggleSource).toContain('const label = enabled ? "启用" : "停用";');
  ...
  it("keeps model service editing details in the shared config component", async () => {
    ...
    expect(sharedSource).toContain("label={service.enabled === false ? \"停用\" : \"启用\"}");
