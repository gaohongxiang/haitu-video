...
describe("video creation layout source", () => {
  ...
  it("routes image creation through a compact product-aware prompt box", async () => {
    ...
    const promptPanelSource = sourceBetween(source, "function StoryboardComposerPanel", "function VideoHistoryPanel");
    const generateProductReferenceImagesSource = sourceBetween(appSource, "async function generateProductReferenceImages", "async function showProviderUsage");
    const handleGenerateVideoSource = sourceBetween(composerSource, "async function handleGenerateVideo", "async function handleGenerateProductImages");
    const handleGenerateProductImagesSource = sourceBetween(composerSource, "async function handleGenerateProductImages", "function selectedReferenceImagesForPromptPreview");
    ...
    expect(source).toContain("type GeneratedReferenceImageResult = { reference: string };");
    expect(appSource).toContain("Promise<GeneratedReferenceImageResult[] | undefined>");
    ...
    expect(appSource).toContain("if (!ensureImageModelConfigured({ toast: false }))");
    expect(appSource).toContain("throw new Error(tApp(\"status.imageModelRequired\"));");
    expect(appSource).toContain("if (response.generated.length === 0)");
    expect(appSource).toContain("throw new Error(tApp(\"status.noGeneratedReferences\"));");
    expect(appSource).toContain("return response.generated;");
    expect(generateProductReferenceImagesSource).not.toContain("showError(error);");
    ...
    expect(composerSource).toContain("const imageModelReady = imageModelOptions.length > 0;");
    expect(composerSource).toContain("const videoModelReady = videoModelOptions.length > 0;");
    expect(handleGenerateVideoSource.indexOf("if (!videoModelReady)")).toBeLessThan(handleGenerateVideoSource.indexOf("setIsSubmittingVideo(true);"));
    expect(handleGenerateProductImagesSource.indexOf("if (!imageModelReady)")).toBeLessThan(handleGenerateProductImagesSource.indexOf("setIsSubmittingImage(true);"));
    expect(handleGenerateProductImagesSource.indexOf("if (!imageModelReady)")).toBeLessThan(handleGenerateProductImagesSource.indexOf("const autoSavedProduct = await onFlushProductFactsAutoSave();"));
    expect(composerSource).toContain("const generatedImages = await onGenerateReferenceImages(savedProduct.sku, {");
    expect(composerSource).toContain("if (!generatedImages || generatedImages.length === 0)");
    expect(composerSource).toContain('onToast(tVideo("generate.imageGenerated", { count: generatedImages.length }), "ok")');
    expect(composerSource).not.toContain('onToast("商品图片优化已提交", "ok")');
  ...
  it("surfaces concise per-action estimates inside stable video creation action buttons", async () => {
    ...
    expect(zhAppText).toContain('"noGeneratedReferences": "图片模型没有生成图片，请调整提示词或参考图后再试。"');
    expect(zhAppText).toContain('"imageGenerated": "已生成 {{count}} 张商品图片。"');
    expect(zhAppText).not.toContain('"imageSubmitted"');
    ...
    expect(enAppText).toContain('"noGeneratedReferences": "The image model did not generate any images. Adjust the prompt or references and try again."');
    expect(enAppText).toContain('"imageGenerated": "Generated {{count}} product images."');
    expect(enAppText).not.toContain('"imageSubmitted"');
