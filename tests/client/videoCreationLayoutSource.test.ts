...
describe("video creation layout source", () => {
  ...
  it("keeps model details summarized behind the compact model scheme control", async () => {
    ...
    expect(settingsTraySource).toContain("model-scheme-control min-w-[96px] max-w-[136px]");
    expect(settingsTraySource).toContain("hidePillLabel");
    expect(settingsTraySource).toContain("localizedModelSchemeChoiceLabel(option, modelSchemeOptions, tVideo)");
    expect(settingsTraySource).toContain("formatActiveLabel={(option) => localizedCompactModelSchemeChoiceLabel(option, modelSchemeOptions, tVideo)}");
