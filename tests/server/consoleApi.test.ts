...
describe("console API", () => {
  ...
  it("serves the React/Tailwind console shell and keeps paid-request safety in the client", async () => {
    ...
    const productStudioFlowSource = await readFile(join(process.cwd(), "src", "client", "productStudioFlow.ts"), "utf8");
    ...
    expect(appSource).toContain("from \"./productStudioFlow.js\"");
    ...
    expect(appShell).toContain('aria-label="刷新控制台数据"');
    expect(appShell).toContain('title="重新拉取最新商品、任务、费用和配置数据"');
    expect(appShell).toContain('size="icon"');
    expect(appShell).toContain('variant="ghost"');
    expect(appShell).toContain("h-8 w-8 rounded-full p-0");
    expect(appShell).not.toContain("h-9 w-9 rounded-full p-0 text-[var(--muted)] shadow-[0_8px_18px_rgba(15,23,42,.05)]");
    expect(appShell).toContain("刷新最新数据");
    expect(appShell).toContain("refresh-tooltip");
    expect(appShell).toContain("group-hover:opacity-100");
    expect(appShell).toContain("重新拉取最新数据");
    ...
    expect(appSource).toContain("资料完整");
    ...
    expect(appSource).toContain("ProductStudio");
    expect(appSource).toContain("product-studio-shell");
    expect(appSource).toContain("product-studio-topbar");
    expect(appSource).toContain("product-studio-title");
    expect(appSource).toContain("product-studio-step-actions");
    expect(appSource).toContain("basis-full");
    expect(appSource).toContain("min-[760px]:basis-auto");
    expect(appSource).toContain("min-[760px]:justify-end");
    expect(appSource).not.toContain("HAITU PRODUCT STUDIO");
    expect(appSource).toContain("product-studio-body");
    expect(appSource).toContain("product-studio-sidebar");
    expect(appSource).toContain("product-studio-main");
    expect(appSource).not.toContain("product-studio-subnav");
    expect(appSource).not.toContain("product-studio-step-toolbar");
    expect(appSource).not.toContain("product-studio-step-bubble");
    expect(appSource).toContain("product-library-shell");
    expect(appSource).toContain("product-library-list");
    expect(appSource).toContain("ProductLibraryDialog");
    expect(appSource).toContain("商品库");
    ...
    const productLibraryDialog = appSource.slice(appSource.indexOf("function ProductLibraryDialog"), appSource.indexOf("function ProductStudioPipeline"));
    ...
    const studioTopbar = appSource.slice(appSource.indexOf("product-studio-topbar"), appSource.indexOf("product-studio-body"));
    const productPickerSource = appSource.slice(appSource.indexOf("function ProductStudioProductPicker"), appSource.indexOf("function VideoCreationEmptyShell"));
    expect(studioTopbar).toContain("product-studio-product-picker");
    expect(studioTopbar).toContain("<ProductStudioProductPicker");
    expect(studioTopbar).toContain("products={studioProductOptions}");
    expect(studioTopbar).toContain("selectedSku={selectedProduct.sku}");
    expect(studioTopbar).not.toContain("<Select");
    expect(productPickerSource).toContain("选择创作商品");
    expect(productPickerSource).toContain("product-studio-product-menu");
    expect(productPickerSource).toContain('aria-haspopup="listbox"');
    expect(productPickerSource).toContain("aria-expanded={productPickerOpen}");
    expect(productPickerSource).toContain('role="listbox"');
    expect(productPickerSource).toContain('role="option"');
    expect(productPickerSource).toContain("onSelectProduct(nextProduct)");
    expect(productPickerSource).toContain("handleProductPickerSelect(NEW_PRODUCT_SELECT_VALUE)");
    expect(productPickerSource).toContain("selectedProductOption.title_ja");
    expect(productPickerSource).toContain("{option.title_ja}");
    expect(productPickerSource).not.toContain("{option.title_ja} / {option.sku}");
    expect(productPickerSource).not.toContain("selectedProductOption.sku");
    expect(productPickerSource).toContain("Plus size={13}");
    expect(productPickerSource).not.toContain("<Select");
    expect(studioTopbar).not.toContain("切换商品");
    expect(studioTopbar).toContain("px-3.5 py-2.5");
    expect(studioTopbar).toContain("product-studio-title");
    expect(studioTopbar).toContain("product-studio-step-actions");
    expect(studioTopbar).toContain("flex min-w-0 flex-1 basis-[460px] flex-nowrap items-center");
    expect(studioTopbar).toContain("max-w-[460px]");
    expect(productPickerSource).toContain("label-inline");
    expect(productPickerSource).not.toContain("text-[22px]");
    expect(studioTopbar).not.toContain("m-0 min-w-0 max-w-full flex-1 truncate");
    expect(studioTopbar).toContain("flex basis-full items-center justify-end");
    expect(studioTopbar).toContain("min-[760px]:ml-auto");
    expect(studioTopbar).toContain("min-[760px]:basis-auto");
    expect(studioTopbar).toContain('aria-label={previousStep ? `上一步：${previousStep.label}` : "已经是第一步"}');
    expect(studioTopbar).toContain('aria-label={nextStep ? `下一步：${nextStep.label}` : "已经是最后一步"}');
    expect(studioTopbar).toContain('title={previousStep ? `上一步：${previousStep.label}` : "已经是第一步"}');
    expect(studioTopbar).toContain('title={nextStep ? `下一步：${nextStep.label}` : "已经是最后一步"}');
    expect(appSource).toContain("const previousStep = steps[activeStepIndex - 1];");
    expect(appSource).toContain("const nextStep = steps[activeStepIndex + 1];");
    expect(studioTopbar).toContain("上一步");
    expect(studioTopbar).toContain("下一步");
    expect(studioTopbar).not.toContain("生成视频");
    expect(studioTopbar).not.toContain("onQueueValidationBatch");
    expect(studioTopbar).not.toContain("onView");
    expect(studioTopbar).not.toContain("刷新");
    const rawStep = appSource.slice(appSource.indexOf('if (activeStep === "raw")'), appSource.indexOf('if (activeStep === "storyboard")'));
    expect(rawStep).not.toContain("手动生成参数");
    const storyboardStep = appSource.slice(appSource.indexOf('if (activeStep === "storyboard")'), appSource.indexOf('if (activeStep === "generate")'));
    expect(storyboardStep).not.toContain("手动生成参数");
    expect(storyboardStep).toContain("<StoryboardDesk");
    expect(storyboardStep).not.toContain("镜头脚本（一行一个镜头）");
    const productsCaseWithoutVideo = appSource.slice(appSource.indexOf('case "products"'), appSource.indexOf('case "video"'));
    expect(productsCaseWithoutVideo).not.toContain("高级参数");
    expect(appSource).toContain("选择创作商品");
    expect(appSource).toContain("NEW_PRODUCT_SELECT_VALUE");
    expect(appSource).not.toContain("切换商品");
    expect(appSource).not.toContain("返回视频创作");
    expect(appSource).not.toContain("创作进度");
    const studioShell = appSource.slice(appSource.indexOf("function ProductStudio"), appSource.indexOf("function ProductLibraryHome"));
    expect(studioShell).not.toContain("activeGroupSteps");
    expect(studioShell).not.toContain("当前阶段");
    expect(studioShell).not.toContain("<TopPill");
    expect(studioShell).not.toContain("steps.map((step) => (");
    expect(studioShell).toContain("studioPipelineCollapsed");
    expect(studioShell).toContain("setStudioPipelineCollapsed");
    expect(studioShell).toContain("min-[1180px]:grid-cols-[56px_minmax(0,1fr)]");
    expect(studioShell).toContain("min-[1180px]:grid-cols-[184px_minmax(0,1fr)]");
    expect(studioShell).not.toContain("min-[1180px]:grid-cols-[64px_minmax(0,1fr)]");
    expect(studioShell).not.toContain("min-[1180px]:grid-cols-[196px_minmax(0,1fr)]");
    expect(studioShell).toContain("product-studio-body grid min-h-[680px]");
    expect(studioShell).not.toContain("product-studio-body grid min-h-[720px]");
    expect(studioShell).toContain("bg-[#f7f9fe] p-4");
    expect(studioShell).not.toContain("bg-[#f7f9fe] p-5");
    expect(studioShell).not.toContain("product-studio-step-bubble");
    expect(studioShell).not.toContain("grid-rows-[minmax(0,1fr)_auto]");
    expect(studioShell).not.toContain("bg-white px-5 py-3");
    expect(studioShell).not.toContain("min-[1180px]:grid-cols-[68px_minmax(0,1fr)]");
    expect(studioShell).not.toContain("min-[1180px]:grid-cols-[220px_minmax(0,1fr)]");
    expect(studioShell).not.toContain("min-[1180px]:grid-cols-[280px_minmax(0,1fr)]");
    expect(studioShell).toContain("const recommendedStep = recommendedProductStudioStep(productStudioFlowState(selectedProduct, selectedProductGroup))");
    expect(studioShell).toContain('const stepKeys = steps.map((step) => step.key).join("|");');
    expect(studioShell).toContain("const activeStepProductSkuRef = useRef<string | undefined>(selectedProduct.sku);");
    expect(studioShell).toContain("productStudioStepFromUrl");
    expect(studioShell).toContain("productStudioStepUrl");
    expect(studioShell).toContain("STUDIO_STEP_STORAGE_KEY");
    expect(appSource).toContain("STUDIO_PRODUCT_SKU_STORAGE_KEY");
    expect(appSource).toContain("productStudioProductSkuFromUrl");
    expect(appSource).toContain("productStudioProductUrl");
    expect(appSource).toContain("function restoreProductStudioSku");
    expect(appSource).toContain("persistProductStudioSku(response.product.sku)");
    expect(appSource).toContain("pendingProductSku={selectedProduct?.sku ?? selectedProductSkuRef.current ?? selectedProductSummary?.sku}");
    expect(studioShell).toContain("function persistProductStudioStep");
    expect(studioShell).toContain("useState<ProductStudioStepKey>(() =>");
    expect(studioShell).toContain("recommendedProductStudioStep(productStudioFlowState(selectedProduct, selectedProductGroup))");
    expect(studioShell).toContain("const productChanged = activeStepProductSkuRef.current !== selectedProduct.sku;");
    expect(studioShell).toContain("setActiveStep((current) => {");
    expect(studioShell).toContain("const currentStepStillAvailable = steps.some((step) => step.key === current);");
    expect(studioShell).toContain("const persistedStep = restoreProductStudioStep(steps, recommendedStep);");
    expect(studioShell).toContain("return persistedStep && steps.some((step) => step.key === persistedStep) ? persistedStep : recommendedStep;");
    expect(studioShell).toContain("function setActiveStepAndPersist(step: ProductStudioStepKey)");
    expect(studioShell).toContain("persistProductStudioStep(step);");
    expect(studioShell).toContain("}, [selectedProduct.sku, recommendedStep, stepKeys]);");
    expect(studioShell).toContain("window.history.replaceState");
    expect(studioShell).not.toContain("}, [selectedProduct.sku, selectedProductGroup?.jobCount, selectedProductGroup?.reviewedJobs, selectedProductGroup?.selectedFinalJobId, productPublishPackages.length])");
    expect(studioShell).not.toContain("selectedProductGroup?.jobCount");
    expect(studioShell).not.toContain("productPublishPackages.length");
    expect(studioShell).not.toContain("recentPublishPackageKey");
    expect(studioShell).not.toContain("highlightedPublishPackageKey={recentPublishPackageKey}");
    expect(appSource).not.toContain("setRecentPublishPackageKey(publishPackageKey(response.package))");
    expect(studioShell).not.toContain('useState<ProductStudioStepKey>("generate")');
    expect(studioShell).not.toContain("recommendedProductStudioStep(selectedProduct, selectedProductGroup)");
    const productStudioFlowStateSource = appSource.slice(appSource.indexOf("function productStudioFlowState"), appSource.indexOf("function productActionSummary"));
    expect(productStudioFlowStateSource).toContain("productFactsReady");
    expect(productStudioFlowStateSource).toContain("referenceImageCount");
    expect(productStudioFlowStateSource).toContain("generatedVersionCount");
    expect(productStudioFlowStateSource).toContain("selectedFinalReady");
    expect(productStudioFlowStateSource).not.toContain("reviewedVersionCount");
    expect(productStudioFlowStateSource).not.toContain("publishPackageCount");
    expect(productStudioFlowSource).toContain('return "raw"');
    expect(productStudioFlowSource).not.toContain('return "images"');
    expect(productStudioFlowSource).toContain('return "storyboard"');
    expect(productStudioFlowSource).toContain('return "generate"');
    expect(productStudioFlowSource).not.toContain('return "review"');
    expect(productStudioFlowSource).not.toContain('return "package"');
    const productStudioStepsSource = appSource.slice(appSource.indexOf("function productStudioSteps"), appSource.indexOf("function productStudioSections"));
    const productStudioSectionsSource = appSource.slice(appSource.indexOf("function productStudioSections"), appSource.indexOf("function productActionSummary"));
    expect(productStudioSectionsSource).toContain("准备");
    expect(productStudioSectionsSource).toContain("创作");
    expect(productStudioSectionsSource).not.toContain("发布");
    expect(productStudioStepsSource).toContain("商品信息");
    expect(productStudioStepsSource).not.toContain('label: "商品资料"');
    expect(productStudioStepsSource).not.toContain("参考图片");
    expect(productStudioStepsSource).toContain("脚本分镜");
    expect(productStudioStepsSource).toContain("生成视频");
    expect(productStudioStepsSource).not.toContain("审核发布");
    expect(productStudioStepsSource).not.toContain("发布素材");
    expect(productStudioStepsSource).not.toContain("数据清洗");
    expect(productStudioStepsSource).not.toContain("创作图片");
    expect(productStudioStepsSource).not.toContain("未来生图候选");
    expect(productStudioStepsSource).not.toContain("生成更优新图");
    expect(productStudioStepsSource).not.toContain("后处理");
    expect(productStudioStepsSource).not.toContain("质检审核");
    expect(productStudioStepsSource).not.toContain('key: "facts"');
    expect(productStudioStepsSource).not.toContain('key: "images"');
    expect(productStudioStepsSource).not.toContain('key: "postprocess"');
    const productStudioPipelineSource = appSource.slice(appSource.indexOf("function ProductStudioPipeline"), appSource.indexOf("function ProductStudioStepPanel"));
    expect(productStudioPipelineSource).toContain("collapsed");
    expect(productStudioPipelineSource).toContain('aria-label={collapsed ? "展开流程轨" : "折叠流程轨"}');
    expect(productStudioPipelineSource).toContain("product-studio-collapse-handle");
    expect(productStudioPipelineSource).toContain("product-studio-collapse-edge");
    expect(productStudioPipelineSource).toContain("product-studio-collapse-thumb");
    expect(productStudioPipelineSource).toContain("product-studio-collapse-chevron");
    expect(productStudioPipelineSource).toContain("product-studio-step-tooltip");
    expect(productStudioPipelineSource).toContain("collapsed ? \"overflow-visible\" : \"overflow-auto\"");
    expect(productStudioPipelineSource).toContain("collapsed ? step.label : undefined");
    expect(productStudioPipelineSource).toContain("group-hover/pipeline-step:opacity-100");
    expect(productStudioPipelineSource).toContain("border-[color-mix(in_srgb,var(--accent)_35%,var(--border))] bg-[color-mix(in_srgb,var(--accent)_10%,white)] text-[var(--text)]");
    expect(productStudioPipelineSource).not.toContain("border-[#b8c8ff] bg-white text-[#244fb9]");
    expect(productStudioPipelineSource).toContain('active ? "bg-transparent text-[var(--text)]" : "bg-[#edf2f8]"');
    expect(productStudioPipelineSource).not.toContain('active ? "bg-[#edf3ff]" : "bg-[#edf2f8]"');
    expect(productStudioPipelineSource).not.toContain("product-studio-collapse-grip");
    const productStudioCollapseHandleSource = productStudioPipelineSource.slice(
      productStudioPipelineSource.indexOf("product-studio-collapse-handle"),
      productStudioPipelineSource.indexOf("product-studio-collapse-edge")
    );
    expect(productStudioCollapseHandleSource).toContain("hover:text-[var(--accent)]");
    expect(productStudioCollapseHandleSource).not.toContain("hover:text-[#244fb9]");
    expect(productStudioCollapseHandleSource).not.toContain("group-hover:opacity-100");
    expect(productStudioCollapseHandleSource).not.toContain("opacity-0");
    expect(productStudioPipelineSource).toContain("absolute inset-y-0 right-[-8px]");
    expect(productStudioPipelineSource).toContain("group-focus-visible:border-[color-mix(in_srgb,var(--accent)_35%,var(--border))]");
    expect(productStudioPipelineSource).not.toContain("group-focus-visible:border-[#b8c8ff]");
    expect(productStudioPipelineSource).toContain("w-4");
    expect(productStudioPipelineSource).toContain('collapsed ? "px-2" : "px-3"');
    expect(productStudioPipelineSource).not.toContain("step.done && !collapsed");
    expect(productStudioPipelineSource).not.toContain("product-studio-step-done");
    expect(productStudioPipelineSource).not.toContain("grid-cols-[32px_minmax(0,1fr)_16px]");
    expect(productStudioPipelineSource).toContain("grid-cols-[32px_minmax(0,1fr)]");
    expect(productStudioPipelineSource).toContain("title={collapsed ? step.label : undefined}");
    expect(productStudioPipelineSource).toContain("sr-only");
    expect(productStudioPipelineSource).not.toContain("pl-2 pr-9");
    expect(productStudioPipelineSource).not.toContain("pl-3 pr-10");
    expect(productStudioPipelineSource).not.toContain("w-7 cursor-pointer border-l");
    expect(productStudioPipelineSource).not.toContain("ChevronsLeft");
    expect(productStudioPipelineSource).not.toContain("ChevronsRight");
    expect(productStudioPipelineSource).not.toContain('collapsed && "absolute translate-x-3 -translate-y-3 rounded-full bg-white"');
    expect(productStudioPipelineSource).not.toContain("mx-auto mb-3 h-9 w-9 rounded-full");
    expect(productStudioPipelineSource).not.toContain("min-h-12 grid-cols-[34px_minmax(0,1fr)_18px]");
    expect(appSource).not.toContain("选择商品进入 Studio");
    expect(appSource).toContain("上一步");
    expect(appSource).toContain("下一步");
    expect(appSource).toContain("参考图");
    expect(appSource).not.toContain("创作准备");
    expect(appSource).toContain("脚本分镜");
    const studioStepPanel = appSource.slice(appSource.indexOf("function ProductStudioStepPanel"), appSource.indexOf("function ProductStudioPanel"));
    const productInfoStep = studioStepPanel.slice(studioStepPanel.indexOf('if (activeStep === "raw")'), studioStepPanel.indexOf('if (activeStep === "storyboard")'));
    expect(productInfoStep).toContain("商品信息");
    expect(productInfoStep).toContain("product.title_ja");
    expect(productInfoStep).not.toContain('label="SKU"');
    expect(productInfoStep).not.toContain("product.sku} />");
    expect(productInfoStep).toContain("参考图");
    expect(productInfoStep).toContain("<ProductReferenceStrip");
    expect(productInfoStep).toContain("编辑资料");
    expect(productInfoStep).toContain("分类");
    expect(productInfoStep).toContain("材质");
    expect(productInfoStep).toContain("尺寸/重量");
    expect(productInfoStep).toContain("已验证卖点");
    expect(productInfoStep).toContain("<ProductSceneTags");
    expect(productInfoStep).toContain("<ProductRiskList");
    const sceneTagsSource = appSource.slice(appSource.indexOf("function ProductSceneTags"), appSource.indexOf("function ProductRiskList"));
    const riskListSource = appSource.slice(appSource.indexOf("function ProductRiskList"), appSource.indexOf("function ProductReferenceStrip"));
    expect(sceneTagsSource).toContain("使用场景");
    expect(riskListSource).toContain("不可用/未确认卖点");
    const referenceStripSource = appSource.slice(appSource.indexOf("function ProductReferenceStrip"), appSource.indexOf("function formatProductInfoText"));
    expect(referenceStripSource).toContain("上传");
    expect(referenceStripSource).toContain("<ReferenceImageFigure");
    expect(referenceStripSource).toContain("onUploadImages(product.sku");
    expect(productInfoStep).toContain("product.verified_selling_points");
    expect(productInfoStep).toContain("product.usage_scenes");
    expect(productInfoStep).toContain("product.forbidden_claims");
    expect(appSource).toContain("function formatProductInfoText");
    expect(appSource).toContain('replace(/\\bSKU\\b/gi, "款式")');
    expect(productInfoStep).not.toContain("创作准备");
    expect(studioStepPanel).not.toContain('if (activeStep === "images")');
    expect(studioStepPanel).not.toContain("审核发布");
    expect(studioStepPanel).not.toContain("发布素材");
    expect(studioStepPanel).not.toContain("发布包");
    expect(studioStepPanel).toContain("生成通道");
    expect(studioStepPanel).toContain("视频时长");
    expect(studioStepPanel).toContain("生成视频");
    expect(studioStepPanel).toContain("允许使用付费模型生成当前商品视频");
    expect(studioStepPanel).not.toContain("预估成本");
    expect(studioStepPanel).not.toContain("原始清单");
    expect(studioStepPanel).not.toContain("最终清单");
    expect(studioStepPanel).not.toContain("manifest");
    expect(studioStepPanel).not.toContain("任务:");
    expect(studioStepPanel).not.toContain("后台记录");
    expect(studioStepPanel).not.toContain("成本明细");
    expect(studioStepPanel).not.toContain("PublishPackageFileStatus");
    expect(studioStepPanel).not.toContain("fileStatus?.video");
    expect(studioStepPanel).not.toContain("fileStatus?.subtitle");
    expect(studioStepPanel).not.toContain("isHighlighted");
    expect(studioStepPanel).not.toContain("highlightedPublishPackageKey === publishPackageKey(pkg)");
    expect(studioStepPanel).not.toContain("技术报告");
    expect(studioStepPanel).not.toContain("gpt-image-2");
    expect(studioStepPanel).toContain("formatOption={providerLabel}");
    expect(studioStepPanel).not.toContain("prompt");
    expect(studioStepPanel).not.toContain("资料可付费生成");
    expect(studioStepPanel).not.toContain("可付费生成");
    expect(studioStepPanel).not.toContain("阻塞原因");
    expect(studioStepPanel).not.toContain("事实分");
    expect(studioStepPanel).not.toContain("<pre");
    expect(studioStepPanel).toContain("参考图");
    expect(studioStepPanel).not.toContain("日语脚本 / 字幕");
    expect(studioStepPanel).toContain("视频分镜");
    expect(studioStepPanel).not.toContain("生成说明");
    expect(studioStepPanel).toContain("历史记录");
    expect(studioStepPanel).toContain("AI 生成分镜");
    expect(studioStepPanel).not.toContain("ReviewVersionRow");
    ["创作模式", "生图候选", "主图清洁化", "生活场景图", "细节特写", "视频首帧图", "广告海报底图"].forEach((label) => {
      expect(studioStepPanel).not.toContain(label);
    });
    expect(studioStepPanel).not.toContain("商品事实包");
    expect(studioStepPanel).not.toContain("后处理");
    expect(studioStepPanel).not.toContain("质检审核");
    expect(studioStepPanel).not.toContain("质检报告");
    expect(studioStepPanel).not.toContain("publishGate.actionEnabled");
    expect(studioStepPanel).not.toContain("onCreatePackage(product.sku)");
    expect(appSource).toContain("detectCompletedVideoJobTransitions");
    expect(appSource).toContain('reason: "polling"');
    expect(appSource).toContain("refreshSelectedProductForStudio");
    expect(appSource).toContain("当前商品创作已刷新");
    expect(studioShell).not.toContain("productPublishPackages");
    expect(studioShell).not.toContain("publishPackages.packages.filter");
    expect(appSource).not.toContain("进入审核发布");
    expect(studioStepPanel).not.toContain("待审核版本");
    expect(studioStepPanel).not.toContain("去审核发布");
    expect(appSource).toContain("selectedProductGroup");
    expect(studioShell).not.toContain("reviewFocusSku");
    expect(studioShell).not.toContain("onOpenReview(product.sku)");
    expect(studioStepPanel).not.toContain("聚焦商品");
    expect(studioStepPanel).not.toContain("查看全部商品版本");
    expect(appSource).toContain("生成 ${selectedVersionCount} 个视频");
    expect(appSource).not.toContain("生成 3 个验证版本");
    expect(appSource).not.toContain("验证版本");
    expect(appSource).not.toContain("可批量验证");
    expect(appSource).not.toContain("批量验证");
    ...
    expect(appSource).not.toContain("onQueueValidationBatch");
    ...
    expect(queueProductVideoJobsSource).toContain("refreshSelectedProductForStudio(product.sku)");
    const generateStepStart = appSource.indexOf('if (activeStep === "generate")');
    const generateStep = appSource.slice(generateStepStart, appSource.indexOf("return null;", generateStepStart));
    expect(generateStep).toContain("<GenerationConsole");
    expect(generateStep).toContain("onProviderChange");
    expect(generateStep).toContain("onVersionCountChange");
    expect(generateStep).toContain("onFinalLanguageChange");
    expect(generateStep).not.toContain("视频风格");
    expect(generateStep).not.toContain("时长（秒）");
    expect(generateStep).not.toContain("CTA");
    expect(generateStep).not.toContain("onDurationChange");
    expect(generateStep).not.toContain("enabledTemplateOptions");
    expect(generateStep).not.toContain("onTemplateChange");
    expect(generateStep).not.toContain("风格会影响脚本结构、镜头节奏和画面表达");
    expect(generateStep).toContain("const needsProductFacts = !actionProduct.importQuality?.ready");
    expect(generateStep).not.toContain("const needsProductFacts = !product.importQuality?.ready");
    expect(generateStep).toContain("needsProductFacts={needsProductFacts}");
    expect(generateStep).toContain("needsReferenceImages={needsReferenceImages}");
    expect(generateStep).toContain("const generateBlockReason");
    expect(generateStep).toContain("const generateDisabled = Boolean(generateBlockReason)");
    expect(generateStep).toContain("onGenerateVideo={onGenerateVideo}");
    expect(generateStep).toContain("latestCreativeJobs={latestCreativeJobs}");
    expect(generateStep).toContain("onCancelVideoJob={onCancelVideoJob}");
    expect(generateStep).not.toContain('onStepChange("images")');
    expect(generateStep).not.toContain("默认 mock");
    expect(generateStep).not.toContain("SKU 入队");
    expect(generateStep).not.toContain("JSON 路径");
    expect(generateStep).not.toContain("高级参数");
    expect(generateStep).not.toContain("生成记录");
    const storyboardDeskSource = appSource.slice(appSource.indexOf("function StoryboardDesk"), appSource.indexOf("function GenerationConsole"));
    expect(storyboardDeskSource).toContain("storyboard-desk");
    expect(storyboardDeskSource).toContain("storyboard-workbench");
    expect(storyboardDeskSource).toContain("storyboard-control-row");
    expect(storyboardDeskSource).toContain("风格");
    expect(storyboardDeskSource).toContain("时长");
    expect(storyboardDeskSource).toContain("StudioOptionSelect");
    expect(storyboardDeskSource).toContain("lg:grid-cols-[minmax(180px,1fr)_minmax(160px,.7fr)_280px]");
    expect(storyboardDeskSource).not.toContain("lg:grid-cols-[minmax(0,1fr)_300px]");
    expect(storyboardDeskSource).toContain("storyboard-editor");
    expect(storyboardDeskSource).toContain("divide-y");
    expect(storyboardDeskSource).not.toContain("lg:divide-x");
    expect(storyboardDeskSource).toContain("视频分镜");
    expect(storyboardDeskSource).not.toContain("生成说明");
    expect(storyboardDeskSource).toContain("历史记录");
    expect(storyboardDeskSource).toContain("storyboard-history");
    expect(storyboardDeskSource).not.toContain("currentLineCount");
    expect(storyboardDeskSource).toContain("onApplyStoryboardHistory");
    expect(storyboardDeskSource).toContain("isGenerating");
    expect(storyboardDeskSource).toContain("生成中");
    expect(storyboardDeskSource).toContain("disabled={isGenerating}");
    expect(storyboardDeskSource).toContain("正在请求文本模型");
    expect(storyboardDeskSource).toContain("generation-hint");
    expect(storyboardDeskSource).toContain("flex min-w-0 items-center gap-2");
    expect(storyboardDeskSource).toContain("generation-hint min-w-0 overflow-hidden text-ellipsis whitespace-nowrap");
    expect(storyboardDeskSource).not.toContain("rounded-[14px] border border-[#dbe4f0] bg-[#f8fbff] p-3");
    expect(storyboardDeskSource).not.toContain("<Select");
    expect(storyboardDeskSource).toContain("补充要点");
    expect(storyboardDeskSource).not.toContain("时间线");
    expect(storyboardDeskSource).not.toContain("镜头脚本");
    expect(storyboardDeskSource).not.toContain("画面/旁白要点");
    expect(storyboardDeskSource).not.toContain("参数面板");
    expect(storyboardDeskSource).not.toContain("director-control");
    expect(storyboardDeskSource).not.toContain("bg-[#172033]");
    const generationConsoleSource = appSource.slice(appSource.indexOf("function GenerationConsole"), appSource.indexOf("function ProductStudioPanel"));
    const generationConsoleComponentSource = appSource.slice(appSource.indexOf("function GenerationConsole"), appSource.indexOf("function ProductStudioPanel"));
    expect(generationConsoleSource).toContain("generation-console");
    expect(generationConsoleSource).toContain("生成控制台");
    expect(generationConsoleSource).toContain("generation-command-bar");
    expect(generationConsoleSource).toContain("handleGenerateVideo");
    expect(generationConsoleSource).toContain("isSubmittingVideo");
    expect(generationConsoleSource).toContain("await onGenerateVideo(actionProduct)");
    expect(generationConsoleSource).toContain("创建生成任务中");
    expect(generationConsoleSource).toContain("已加入历史记录");
    expect(generationConsoleSource).toContain("generateBlockReason");
    expect(generationConsoleSource).toContain("generation-workbench");
    expect(generationConsoleSource).toContain("generation-control-row");
    expect(generationConsoleSource).toContain("StudioOptionSelect");
    expect(generationConsoleSource).toContain("成片语言");
    expect(generationConsoleSource).toContain("formatOption={finalLanguageLabel}");
    expect(generationConsoleSource).not.toContain("generation-parameter-strip");
    expect(generationConsoleSource).not.toContain("生成参数");
    expect(generationConsoleSource).not.toContain("只改执行参数");
    expect(generationConsoleSource).not.toContain("生成通道 / 模型");
    expect(generationConsoleSource).not.toContain("确认参数后即可创建当前商品的视频生成任务。");
    expect(generationConsoleComponentSource).not.toContain("bg-[#172033]");
    expect(generationConsoleComponentSource).not.toContain("执行设置");
    expect(generationConsoleSource).toContain("视频类型和时长来自脚本分镜步骤");
    expect(generationConsoleSource).toContain("资料还没整理完整");
    expect(generationConsoleSource).toContain("编辑资料");
    expect(generationConsoleSource).toContain("参考图不足时先回到商品信息补图");
    expect(generationConsoleSource).toContain("历史记录");
    expect(generationConsoleComponentSource).toContain("generation-history-scroll");
    expect(generationConsoleComponentSource).toContain("max-h-[360px]");
    expect(generationConsoleComponentSource).toContain("overflow-y-auto");
    expect(generationConsoleComponentSource).not.toContain("最近版本");
    expect(generationConsoleComponentSource).not.toContain("最近视频");
    expect(generationConsoleComponentSource).not.toContain("生成版本");
    expect(generationConsoleComponentSource).not.toContain("视频版本");
    expect(generationConsoleSource).toContain("videoLabel(index)");
    expect(generationConsoleSource).toContain("isActiveCreativeVersion");
    expect(generationConsoleSource).not.toContain("isFreshCreativeVersion");
    expect(generationConsoleSource).toContain("formatCreativeVersionTime");
    expect(generationConsoleSource).toContain("RefreshCcw");
    expect(generationConsoleSource).toContain("animate-spin");
    expect(generationConsoleSource).toContain("handleDeleteCreativeVersion");
    expect(generationConsoleSource).toContain("DeleteCreativeVersionDialog");
    expect(generationConsoleSource).toContain("requestDeleteCreativeVersion");
    expect(generationConsoleSource).toContain("onDeleteLedgerVideo");
    expect(generationConsoleSource).toContain("job.source === \"video-job\"");
    expect(generationConsoleSource).toContain("handleCancelPreviewJob");
    expect(generationConsoleSource).toContain("requestDeleteCreativeVersion(job)");
    expect(generationConsoleSource).toContain("onSelectFinal(actionProduct.sku, job.id)");
    expect(generationConsoleSource).toContain("retryJob");
    expect(generationConsoleSource).toContain("onRetryVideoJob(retryJob)");
    expect(generationConsoleSource).toContain("previewJob");
    expect(generationConsoleSource).toContain("setPreviewJob(job)");
    expect(generationConsoleSource).toContain("<VideoPreviewDialog");
    expect(generationConsoleSource).toContain("预览视频");
    expect(generationConsoleSource).toContain("下载视频");
    expect(generationConsoleSource).toContain("download");
    expect(generationConsoleSource).toContain("设为最终");
    expect(generationConsoleSource).toContain("重试");
    expect(generationConsoleSource).toContain("finalVideoUrl");
    expect(generationConsoleSource).toContain("hasPlayableVideo(job)");
    expect(generationConsoleSource).not.toContain("canPreview");
    expect(generationConsoleSource).not.toContain("target=\"_blank\"");
    expect(generationConsoleSource).not.toContain("刚生成");
    expect(generationConsoleSource).toContain("删除");
    expect(generationConsoleSource).toContain("确认删除");
    expect(generationConsoleSource).toContain("本地输出目录也会一起删除");
    expect(appSource).toContain("/api/job-ledger/${encodeURIComponent(jobId)}");
    expect(appSource).toContain("已删除历史视频记录");
    expect(appSource).toContain("\"video_history.deleted\": \"删除历史视频\"");
    const latestCreativeJobsSource = appSource.slice(appSource.indexOf("function buildLatestCreativeJobs"), appSource.indexOf("function ledgerJobToCreativeVersion"));
    expect(latestCreativeJobsSource).toContain("const matchingVideoJobs");
    expect(latestCreativeJobsSource).toContain("new Set(matchingVideoJobs.map((job) => job.id))");
    expect(latestCreativeJobsSource).toContain("hasPlayableVideo(job)");
    expect(latestCreativeJobsSource).not.toContain("job.status === \"completed\"");
    expect(latestCreativeJobsSource).not.toContain(".slice(0, 4)");
    expect(generationConsoleSource).toContain("商品信息");
    expect(generationConsoleSource).toContain("onEdit(actionProduct.sku)");
    expect(generationConsoleSource).toContain('onStepChange("raw")');
    expect(generationConsoleSource).not.toContain("onQueueValidationBatch");
    expect(queueProductVideoJobsSource).toContain("已开始制作");
    expect(queueProductVideoJobsSource).toContain("个视频");
    ...
    expect(queueProductVideoJobsSource).not.toContain("按商品 SKU 入队");
    expect(queueProductVideoJobsSource).not.toContain("job.id");
    expect(queueProductVideoJobsSource).not.toContain("job.outDir");
    expect(appSource).not.toContain("生成视频会使用这里保存的草稿");
    expect(storyboardDeskSource).toContain("视频风格");
    expect(storyboardDeskSource).toContain("视频时长");
    expect(storyboardStep).toContain("onDurationChange");
    expect(storyboardStep).toContain("onTemplateChange");
    expect(storyboardDeskSource).toContain("templateLabel");
    expect(storyboardDeskSource).not.toContain("CTA");
    expect(storyboardDeskSource).not.toContain("字幕");
    expect(appSource).toContain("scriptLines: splitDraftLines(studioScriptDraft)");
    expect(appSource).toContain("storyboardLines: splitDraftLines(studioStoryboardDraft)");
    ...
    expect(appSource).toContain("saveStoryboardHistory(persistedHistory)");
    ...
    const applyStoryboardHistorySource = appSource.slice(appSource.indexOf("function applyStoryboardHistory"), appSource.indexOf("function productTitleForSku"));
    expect(applyStoryboardHistorySource).toContain("setTemplate(record.template)");
    expect(applyStoryboardHistorySource).toContain("setDuration(record.duration)");
    const defaultStoryboardSource = appSource.slice(appSource.indexOf("function defaultStudioScriptDraft"), appSource.indexOf("function splitList"));
    expect(defaultStoryboardSource).toContain("safeChineseDraftFact");
    expect(defaultStoryboardSource).toContain("safeChineseDraftText");
    expect(defaultStoryboardSource).not.toContain("product.verified_selling_points[0] || \"商品细节\"");
    expect(appSource).toContain("onGenerateVideo");
    expect(appSource).toContain("referenceReadiness");
    expect(appSource).toContain("补 2 张参考图后可生成视频");
    expect(appSource).toContain('setActiveSection("video")');
    expect(appSource).toContain('postJson<{ product: ProductDetail }>("/api/products"');
    expect(appSource).toContain("上传参考图");
    expect(appSource).toContain("/reference-images");
    expect(appSource).toContain("/api/video-jobs");
    expect(appSource).toContain("/api/video-jobs/batch");
    expect(appSource).toContain("视频数量");
    expect(appSource).toContain("versionCount");
    expect(appSource).not.toContain("批量预估成本");
    expect(appSource).not.toContain("版本数会放大成本");
    expect(appSource).toContain("batchEstimatedCostCny");
    expect(appSource).toContain("生成任务记录");
    expect(appSource).not.toContain("异步任务");
    expect(appSource).toContain("/cancel");
    expect(appSource).toContain("buildLatestCreativeJobs");
    expect(appSource).toContain("videoJobs={videoJobs}");
    expect(appSource).toContain("onCancelVideoJob={cancelVideoJob}");
    ...
    expect(ledgerCase).toContain("<ProviderUsagePanel");
    expect(ledgerCase).toContain("<FeeSummaryPanel");
    expect(ledgerCase).toContain("<VideoJobsPanel");
    expect(ledgerCase).toContain("<ReportsPanel");
    expect(ledgerCase).not.toContain("<PublishPackagesPanel");
    expect(ledgerCase).toContain("<StorageBackupPanel");
    expect(ledgerCase).toContain("<AuditLogPanel");
    ...
    expect(appSource).toContain("加载商品创作工作台失败");
    expect(appSource).toContain("重试加载");
    expect(appSource).toContain("ProductFactSummaryStrip");
    expect(appSource).toContain("ProductNarrativeList");
    expect(appSource).toContain("ProductSceneTags");
    expect(appSource).toContain("ProductRiskList");
    expect(appSource).toContain("product-reference-strip");
  ...
  it("separates product management and video creation as the primary modules", async () => {
    ...
    const productCreationWorkspace = appSource.slice(appSource.indexOf("function ProductCreationWorkspace"), appSource.indexOf("type ProductStudioSection"));
    expect(productCreationWorkspace).toContain("<ProductStudio");
    ...
    expect(productCreationWorkspace).toContain("ensureVideoProductSelection");
    ...
    expect(appSource).not.toContain("product-creation-picker");
    ...
    expect(studioTopbar).not.toContain("切换商品");
    expect(studioTopbar).not.toContain("返回视频创作");
    expect(studioTopbar).not.toContain("返回商品项目");
  });
