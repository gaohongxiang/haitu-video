...
describe("console API", () => {
  ...
  it("serves the React/Tailwind console shell and keeps paid-request safety in the client", async () => {
    ...
    expect(creationComposerSource).toContain("Math.max(14, Math.min(20");
    expect(creationComposerSource).toContain("product-facts-body h-full min-h-[520px]");
  ...
  it("renders video creation as one composer with inline product packing, controls, storyboard, and video history", async () => {
    ...
    expect(composerSource).toContain("Math.max(14, Math.min(20");
    expect(composerSource).toContain("product-facts-body h-full min-h-[520px]");
