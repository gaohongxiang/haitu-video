...
describe("model config presentation source boundaries", () => {
  it("centralizes provider config ledger and admin presentation builders", async () => {
    ...
    expect(presentationSource).toContain("export async function buildModelServiceAdminConfig(");
    ...
    expect(presentationSource).toContain("interface ModelServiceAdminConfigResponse");
  });
});
