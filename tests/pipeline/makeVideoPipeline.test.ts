...
describe("runMakeVideoPipeline", () => {
  it("does not fall back to product images when no reference image selection is provided", async () => {
    const root = await mkdtemp(join(tmpdir(), "haitu-make-video-no-refs-"));
    tempDirs.push(root);
